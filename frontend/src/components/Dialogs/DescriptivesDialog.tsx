...
export function DescriptivesDialog({ open, onClose }: Props) {
  ...
  const handleRun = async () => {
    if (!sessionId || selectedVars.length === 0) { toast.error("Select at least one variable"); return; }
    setLoading(true);
    try {
      const result = await descriptivesApi.descriptives(sessionId, selectedVars);
      addOutputBlock({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white border border-gray-300 shadow-xl rounded w-96 p-4">
        <h3 className="font-bold text-sm mb-3">Descriptive Statistics</h3>
        <div className="mb-3">
          <label className="text-xs text-gray-600 block mb-1">Variables:</label>
          <div className="border border-gray-300 rounded h-48 overflow-auto p-1">
            {metadata?.variables
