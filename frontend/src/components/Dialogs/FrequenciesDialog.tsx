...
export function FrequenciesDialog({ open, onClose }: Props) {
  ...
  const handleRun = async () => {
    ...
    setLoading(true);
    try {
      const result = await descriptivesApi.frequencies(sessionId, selected, { bar_chart: showBarChart });
      addOutputBlock({
        id: crypto.randomUUID(),
        type: "table",
        title: `Frequencies — ${selected}`,
        content: result,
        created_at: new Date(),
      ...
      toast.error(err instanceof Error ? err.message : "Error running frequencies");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white border border-gray-300 shadow-xl rounded w-80 p-4">
        <h3 className="font-bold text-sm mb-3">Frequencies</h3>
        <div className="mb-3">
          <label className="text-xs text-gray-600 block mb-1">Variable:</label>
          <select
